            ...
            <div class="py-4 sm:p-8">
                <div class="w-full">
                    <section>
                        ...
                        <form method="POST" action="{{ route('rsenv.update', $rsenv->id) }}" class="mt-6 space-y-6">
                            @csrf
                            @method('put')
                            ...
                            <div>
                                <div class="flex flex-row text-center items-center">
                                    ...
                                    <x-text-input id="date" name="date" type="date" class="block w-full h-8"
                                        :value="old('date', $rsenv->date)" required />
                                </div>
                            ...
                            <div>
                                <div class="flex flex-row text-center items-center">
                                    ...
                                    <select name="validation" id="validation"
                                        ...
                                        <option value="0" {{ $rsenv->validation === 0 ? 'selected' : '' }}>
                                        </option>
                                        <option value="1" {{ $rsenv->validation === 1 ? 'selected' : '' }}>OUI
                                        </option>
                                        <option value="2" {{ $rsenv->validation === 2 ? 'selected' : '' }}>NON
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div>
                                <div class="flex flex-row">
                                    ...
                                    <textarea class="border border-gray-300 border-solid w-full" name="observations" id="observations" rows="5">{{ old('observations', $rsenv->observations) }}</textarea>
                                </div>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                @if (session('status') === 'rsenv-updated')
